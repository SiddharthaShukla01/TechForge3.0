...
# ─────────────────────────────────────────────────────────────────────────────
# 7. ADMIN & STATE EMERGENCY OPERATIONS COMMAND (SEOC) DASHBOARD
# ─────────────────────────────────────────────────────────────────────────────
elif selected_nav == t("nav_admin", lang):
    st.markdown(f'<div class="portal-title">{"⚙️ राज्य आपदा नियंत्रण कक्ष एवं अधिकारी पैनल" if is_hi else "⚙️ State Emergency Operations Command & Admin Center"}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="portal-subtitle">{"जिलाधिकारियों, एसडीआरएफ कमान और राहत आयुक्त के लिए एकीकृत नियंत्रण एवं संपादन प्रणाली।" if is_hi else "Unified Command, Control, Editing, and Reporting Dashboard for District Magistrates & SDRF Incident Commanders."}</div>', unsafe_allow_html=True)
    ...
    if not st.session_state["admin_authenticated"]:
        ...
        <div style="background:rgba(15,23,42,0.85);border:1px solid rgba(59,130,246,0.3);border-radius:14px;padding:24px;max-width:550px;margin:20px auto 40px;">
            <div style="font-size:1.1rem;font-weight:800;color:#F8FAFC;margin-bottom:8px;display:flex;align-items:center;gap:8px;">
                🔒 {'कंट्रोल रूम सुरक्षा सत्यापन' if is_hi else 'Control Room Security Access Gate'}
            </div>
                        ...
                        with st.form("dispatch_rescue_form"):
                            ...
                            with dc1:
                                team_name = st.selectbox("Rescue Unit / Battalion:", [
                                    ...
                                    "SDRF Mountain Rescue (Rudraprayag Base)",
                                    "NDRF 15th Battalion (Regional Response Center)",
                                    "ITBP Rapid Disaster Response Wing",
                                    "Uttarakhand Police Quick Response Team (QRT)"
                                ])
                            with dc2:
                                personnel_cnt = st.number_input("Personnel Count (जवानों की संख्या):", min_value=4, max_value=120, value=20)

                            dc3, dc4 = st.columns(2)
                            with dc3:
                                leader_name = st.text_input("Team Commander Name (दस्ता प्रभारी):", value="Insp. Vikram Negi")
                            with dc4:
                                leader_phone = st.text_input("Commander Wireless / Phone (संपर्क नंबर):", value="9456596190")

                            disp_notes = st.text_area("Deployment Instructions & Equipment Dispatched:", value="Equipped with inflatable boats, rope rescue gear, satellite phone, and trauma kits.")

                            btn_dispatch = st.form_submit_button("🚨 " + ("दस्ता रवाना करें एवं लाइव अलर्ट जारी करें" if is_hi else "Authorize Deployment & Broadcast Alert"), type="primary")
                            if btn_dispatch:
                                dispatch_rescue_team(sel_disp_id, team_name, leader_name, leader_phone, personnel_cnt, disp_notes)
                                st.success(f"Rescue unit deployed to Incident #{sel_disp_id} and broadcast alert issued!")
                                st.rerun()
                    else:
                        st.info("No active unresolved incidents currently requiring rescue deployment.")

                # LOG NEW INCIDENT FROM COMMAND
                with inc_act_add:
                    st.markdown("##### ➕ " + ("कंट्रोल रूम से नई आपदा दर्ज करें" if is_hi else "Direct Command Incident Logging"))
                    with st.form("admin_new_incident_form"):
                        ac1, ac2, ac3 = st.columns(3)
                        with ac1:
                            new_dtype = st.selectbox("Disaster Type:", list(DISASTER_TYPE_TRANSLATIONS.keys()), key="adm_new_dtype")
                        with ac2:
                            new_loc = st.selectbox("District:", ALL_DISTRICTS, key="adm_new_loc")
                        with ac3:
                            new_sev = st.selectbox("Severity:", ["Critical", "High", "Medium", "Low"], index=1, key="adm_new_sev")

                        new_desc = st.text_area("Situation Description:", placeholder="Enter verified incident information received from district control room...")
                        new_contact = st.text_input("Nodal Contact:", value="District Control Room 1077")

                        btn_add_inc = st.form_submit_button("🚨 " + ("घटना दर्ज करें" if is_hi else "Submit Incident"), type="primary")
                        if btn_add_inc:
                            nid = add_disaster(new_dtype, new_loc, new_sev, new_desc, reporter_contact=new_contact)
                            st.success(f"New emergency incident #{nid} created and auto-broadcasted!")
                            st.rerun()

                # DELETE INCIDENT
                with inc_act_del:
                    st.markdown("##### 🗑️ " + ("फर्जी, निरस्त या डुप्लीकेट रिपोर्ट हटाएं" if is_hi else "Purge Spam or Duplicate Report"))
                    options_del = {row['id']: f"#{row['id']} — {row['type']} in {row['location']}" for _, row in df_all.iterrows()}
                    sel_del_id = st.selectbox("Select Incident to Delete:", options_del.keys(), format_func=lambda x: options_del[x], key="adm_del_sel")
                    st.warning(f"⚠️ Are you sure you want to permanently delete Incident #{sel_del_id} and its associated alert feed?")
                    if st.button("🗑️ " + ("स्थायी रूप से हटाएं (Confirm Delete)" if is_hi else "Permanently Delete Incident"), type="secondary"):
                        delete_disaster(sel_del_id)
                        st.success(f"Incident #{sel_del_id} permanently removed!")
                        st.rerun()
            else:
                st.info("No recorded incidents.")

        # ─────────────────────────────────────────────────────────────────────
        # TAB 2: HOSPITALS & MEDICAL BEDS MATRIX
        # ─────────────────────────────────────────────────────────────────────
        with ad_tab2:
            st.subheader("🏥 " + ("अस्पताल एवं बेड नियंत्रण मैट्रिक्स" if is_hi else "Hospital Capacity & Medical Matrix"))
            hospitals_df = get_hospitals_by_district()

            if not hospitals_df.empty:
                st.dataframe(hospitals_df[['id', 'name', 'district', 'beds_available', 'beds_total', 'contact']], use_container_width=True, hide_index=True)

                h_act_tab1, h_act_tab2, h_act_tab3 = st.tabs([
                    "⚡ " + ("त्वरित बेड संख्या अपडेट" if is_hi else "Quick Bed Count Updater"),
                    "➕ " + ("नया अस्पताल / फील्ड यूनिट जोड़ें" if is_hi else "Add Hospital / Field Clinic"),
                    "🗑️ " + ("अस्पताल हटाएं" if is_hi else "Delete Facility")
                ])

                with h_act_tab1:
                    h_opt = {row['id']: f"{row['name']} ({row['district']}) — Free: {row['beds_available']}/{row['beds_total']}" for _, row in hospitals_df.iterrows()}
                    sel_hid = st.selectbox("Select Hospital Facility:", h_opt.keys(), format_func=lambda x: h_opt[x], key="adm_h_sel")
                    curr_h = hospitals_df.loc[hospitals_df['id'] == sel_hid].iloc[0]

                    with st.form("quick_h_bed_form"):
                        hc1, hc2 = st.columns(2)
                        with hc1:
                            new_h_avail = st.number_input("Available Free Beds (खाली बेड):", min_value=0, max_value=int(curr_h['beds_total']), value=int(curr_h['beds_available']))
                        with hc2:
                            new_h_tot = st.number_input("Total Bed Capacity (कुल बेड):", min_value=1, max_value=5000, value=int(curr_h['beds_total']))

                        h_save_btn = st.form_submit_button("💾 " + ("बेड संख्या सुरक्षित करें" if is_hi else "Save Bed Availability"), type="primary")
                        if h_save_btn:
                            edit_hospital(sel_hid, curr_h['name'], curr_h.get('name_hi', curr_h['name']), curr_h['district'], curr_h.get('district_hi', curr_h['district']), new_h_avail, new_h_tot, curr_h['contact'])
                            st.success("Hospital bed capacity updated!")
                            st.rerun()

                with h_act_tab2:
                    with st.form("add_new_hospital_form"):
                        ...
                        with ahc2:
                            new_h_name_hi = st.text_input("Hospital Name (Hindi):", placeholder="उदा: एम्स ऋषिकेश आपातकालीन विंग")
                            new_h_phone = st.text_input("Emergency Contact Number:", placeholder="0135-2462000")

                        ahc3, ahc4 = st.columns(2)
                        with ahc3:
                            add_h_tot = st.number_input("Total Beds:", min_value=1, max_value=5000, value=100)
                        with ahc4:
                            add_h_avail = st.number_input("Initial Free Beds:", min_value=0, max_value=5000, value=40)

                        if st.form_submit_button("➕ " + ("अस्पताल जोड़ें" if is_hi else "Register Hospital"), type="primary"):
                            if new_h_name.strip():
                                add_hospital(new_h_name.strip(), new_h_name_hi.strip() or new_h_name.strip(), new_h_dist, new_h_dist, add_h_avail, add_h_tot, new_h_phone)
                                st.success("New hospital registered!")
                                st.rerun()

                with h_act_tab3:
                    sel_del_h = st.selectbox("Select Facility to Remove:", h_opt.keys(), format_func=lambda x: h_opt[x], key="adm_h_del_sel")
                    if st.button("🗑️ " + ("अस्पताल हटाएं (Confirm Delete)" if is_hi else "Delete Hospital Facility"), type="secondary"):
                        ...
                        st.success("Hospital facility deleted.")
                        st.rerun()

        # ─────────────────────────────────────────────────────────────────────
        # TAB 3: RELIEF SHELTERS & EVACUATION CAMPS
        # ─────────────────────────────────────────────────────────────────────
        with ad_tab3:
            ...
            if not shelters_df.empty:
                st.dataframe(shelters_df[['id', 'name', 'district', 'occupied', 'capacity', 'contact']], use_container_width=True, hide_index=True)

                s_act_tab1, s_act_tab2, s_act_tab3 = st.tabs([
                    "⚡ " + ("ऑक्यूपेंसी संख्या अपडेट करें" if is_hi else "Update Occupancy"),
                    "➕ " + ("नया राहत शिविर जोड़ें" if is_hi else "Register New Shelter Camp"),
                    "🗑️ " + ("शिविर हटाएं" if is_hi else "Delete Shelter")
                ])

                with s_act_tab1:
                    s_opt = {row['id']: f"{row['name']} ({row['district']}) — Occupied: {row['occupied']}/{row['capacity']}" for _, row in shelters_df.iterrows()}
                    sel_sid = st.selectbox("Select Relief Camp:", s_opt.keys(), format_func=lambda x: s_opt[x], key="adm_s_sel")
                    curr_s = shelters_df.loc[shelters_df['id'] == sel_sid].iloc[0]

                    with st.form("quick_s_occ_form"):
                        new_s_occ = st.number_input("Occupied Person Count (भरे हुए स्थान):", min_value=0, max_value=int(curr_s['capacity']), value=int(curr_s['occupied']))
                        if st.form_submit_button("💾 " + ("ऑक्यूपेंसी सुरक्षित करें" if is_hi else "Save Shelter Occupancy"), type="primary"):
                            update_shelter_occupancy(sel_sid, new_s_occ)
                ...
                with s_act_tab2:
                    with st.form("add_new_shelter_form"):
                        ...
                        with asc1:
                            new_s_name = st.text_input("Shelter Name (English):", placeholder="e.g. Joshimath Community Hall Camp")
                            new_s_dist = st.selectbox("District:", ALL_DISTRICTS, key="adm_add_s_dist")
                        with asc2:
                            new_s_name_hi = st.text_input("Shelter Name (Hindi):", placeholder="उदा: जोशीमठ सामुदायिक भवन राहत शिविर")
                            new_s_phone = st.text_input("Camp Incharge Contact Number:", placeholder="01372-222333")

                        new_s_cap = st.number_input("Total Capacity (व्यक्तियों की क्षमता):", min_value=10, max_value=10000, value=200)

                        if st.form_submit_button("➕ " + ("राहत शिविर जोड़ें" if is_hi else "Register Shelter"), type="primary"):
                            if new_s_name.strip():
                ...
                with s_act_tab3:
                    ...
                    if st.button("🗑️ " + ("शिविर हटाएं (Confirm Delete)" if is_hi else "Delete Shelter Camp"), type="secondary"):
                        ...
                        st.success("Shelter camp deleted.")
                        st.rerun()

        # ─────────────────────────────────────────────────────────────────────
        # TAB 4: SUPPLY STOCKPILES & WAREHOUSE LOGISTICS
        # ─────────────────────────────────────────────────────────────────────
        with ad_tab4:
            st.subheader("📦 " + ("राशन, पेयजल व राहत सामग्री भंडार" if is_hi else "Relief Supplies & Warehouse Stockpiles"))
            res_df = get_resources_by_district()

            if not res_df.empty:
                st.dataframe(res_df[['id', 'type', 'name', 'district', 'quantity', 'unit']], use_container_width=True, hide_index=True)

                r_act_tab1, r_act_tab2, r_act_tab3 = st.tabs([
                    "⚡ " + ("स्टॉक स्तर अपडेट करें" if is_hi else "Update Stock Quantity"),
                    "➕ " + ("नई राहत सामग्री जोड़ें" if is_hi else "Add New Stockpile Entry"),
                    "🗑️ " + ("स्टॉक एंट्री हटाएं" if is_hi else "Delete Stock Entry")
                ])

                with r_act_tab1:
                    r_opt = {row['id']: f"{row['name']} ({row['district']}) — Qty: {row['quantity']} {row['unit']}" for _, row in res_df.iterrows()}
